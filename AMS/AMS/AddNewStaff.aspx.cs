...
namespace AMS
{
    public partial class AddNewStaff : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            string qry = StaffCreate.AddNewStaff(SFnameBox.Text, SSnameBox.Text, DeptTextBox.Text, UnameBox.Text, PwsdTextBox.Text);
            if (qry == null)
                SlblMsg.Text = "You have successfully created a new Staff";
            else
            {
        ...
        public class StaffCreate
        {
            ...
            public static string AddNewStaff(string fname, string Surname, string pwd,string fcode, string dept  )
            {
                SqlConnection con = new SqlConnection(Database.ConnectionString);
                try
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("Insert into faculty(fname,Surname,fcode,dept,pwd) values (@fname,@Surname,@fcode,@dept,@pwd)", con);
                    cmd.Parameters.Add("@fname", SqlDbType.VarChar).Value = fname;
                    cmd.Parameters.Add("@Surname", SqlDbType.VarChar).Value = Surname;
                    cmd.Parameters.Add("@fcode", SqlDbType.VarChar).Value = fcode;
                    cmd.Parameters.Add("@dept", SqlDbType.Int).Value = Convert.ToInt32(dept);
